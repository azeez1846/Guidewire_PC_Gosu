...
public class PcfStudioServlet extends HttpServlet {
    ...
    private void serveStudioUI(HttpServletResponse resp) throws IOException {
        ...
        out.println("    .actions { display: flex; gap: 10px; align-items: center; }");
        ...
        out.println("    button.purple { background: #7c3aed; }");
        ...
        out.println("    /* UI Live Mode */");
        out.println("    .live-preview-card { background: white; color: #1e293b; border-radius: 8px; padding: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); margin-bottom: 16px; }");
        out.println("    .live-field { margin-bottom: 12px; display: flex; flex-direction: column; gap: 4px; }");
        out.println("    .live-field label { font-size: 0.8rem; font-weight: 600; color: #475569; }");
        out.println("    .live-field input, .live-field select { padding: 6px 10px; border: 1px solid #cbd5e1; border-radius: 4px; font-size: 0.85rem; }");
        out.println("  </style>");
        out.println("</head>");
        out.println("<body>");
        out.println("  <header>");
        ...
        out.println("      <button class=\"purple\" onclick=\"cloneSelectedPcf()\">👯 Clone / Create From Template</button>");
        out.println("      <button class=\"secondary\" onclick=\"createNewPcf()\">+ Blank PCF</button>");
        ...
        out.println("        <span id=\"canvasPcfName\">Editing: WorkersCompCoveragesDVTile.pcf</span>");
        out.println("        <div>");
        out.println("          <button class=\"secondary\" onclick=\"toggleViewMode()\" id=\"btnViewMode\">👁️ Switch to Live UI Preview</button>");
        out.println("          <span style=\"margin-left:12px;\">Schema Validation Guard: <strong style=\"color:#22c55e;\">ENFORCED</strong></span>");
        out.println("        </div>");
        out.println("      </div>");
        ...
        out.println("      id: 'WorkersCompCoveragesDVTile',");
        out.println("      title: 'Workers Compensation Coverages',");
        out.println("      children: [");
        out.println("        { type: 'DetailViewPanel', id: 'WCLineDV', children: [");
        out.println("            { type: 'InputColumn', id: 'Col1', title: 'Employers Liability Limits', children: [");
        out.println("                { type: 'SelectInput', id: 'EmployersLiabilityLimit', label: 'Employers Liability Limit', value: 'WCPolicyLine.EmployersLiabilityLimit', required: 'true' },");
        out.println("                { type: 'CheckBoxInput', id: 'OtherStatesCoverage', label: 'Other States Coverage (Part Three)', value: 'WCPolicyLine.OtherStatesCoverage' },");
        out.println("                { type: 'CheckBoxInput', id: 'USLHCoverage', label: 'US Longshore & Harbor Workers (USL&H)', value: 'WCPolicyLine.USLHCoverage' }");
        out.println("            ]}");
        out.println("          ]}");
        out.println("      ]");
        out.println("    };");
        out.println("    let selectedNode = null;");
        out.println("    let isLiveMode = false;");
        ...
        out.println("        sel.innerHTML = '<option value=\"\">-- Select Existing PCF --</option>';");
        ...
        out.println("    function toggleViewMode() {");
        out.println("      isLiveMode = !isLiveMode;");
        out.println("      document.getElementById('btnViewMode').innerText = isLiveMode ? '🏗️ Switch to Canvas Tree' : '👁️ Switch to Live UI Preview';");
        ...
        out.println("      if (isLiveMode) {");
        out.println("        canvas.appendChild(renderLivePreview(currentPcf));");
        out.println("      } else {");
        out.println("        currentPcf.children.forEach(c => canvas.appendChild(createNodeElem(c, currentPcf)));");
        out.println("      }");
        out.println("      updateXmlPreview();");
        out.println("    }");
        out.println("    function renderLivePreview(pcf) {");
        out.println("      const card = document.createElement('div'); card.className = 'live-preview-card';");
        out.println("      card.innerHTML = `<h3 style=\"margin-bottom:16px; border-bottom:1px solid #e2e8f0; padding-bottom:8px;\">${pcf.title || pcf.id}</h3>`;");
        out.println("      const renderNodes = (nodes, target) => {");
        out.println("        if (!nodes) return;");
        out.println("        nodes.forEach(n => {");
        out.println("          if (n.type === 'TextInput' || n.type === 'DateInput') {");
        out.println("            target.innerHTML += `<div class=\"live-field\"><label>${n.label || n.id} ${n.required === 'true' ? '<span style=\"color:red\">*</span>':''}</label><input type=\"text\" value=\"${n.value || ''}\" placeholder=\"Binding: ${n.value || ''}\"></div>`;");
        out.println("          } else if (n.type === 'SelectInput') {");
        out.println("            target.innerHTML += `<div class=\"live-field\"><label>${n.label || n.id}</label><select><option>${n.value || 'Select Option...'}</option></select></div>`;");
        out.println("          } else if (n.type === 'CheckBoxInput') {");
        out.println("            target.innerHTML += `<div style=\"margin-bottom:8px;\"><label><input type=\"checkbox\" checked> ${n.label || n.id}</label></div>`;");
        out.println("          } else if (n.children) { renderNodes(n.children, target); }");
        out.println("        });");
        out.println("      };");
        out.println("      renderNodes(pcf.children, card);");
        out.println("      return card;");
        out.println("    }");
        ...
        out.println("      }");
        out.println("    }");
        out.println("    function onDragOverContainer(ev, parentType, elem) { ev.preventDefault(); elem.classList.add('drag-over-valid'); }");
        out.println("    function onDropOnContainer(ev, parentNode) {");
        out.println("      ev.preventDefault(); ev.stopPropagation();");
        ...
        out.println("      const t = document.getElementById('errorToast'); t.innerText = msg; t.style.display = 'block';");
        ...
        out.println("    function updateXmlPreview() { document.getElementById('xmlPreview').innerText = generateXml(currentPcf); }");
        ...
        out.println("        alert(res.message); loadPcfFileList();");
        out.println("      });");
        out.println("    }");
        ...
        out.println("        parseXmlToTree(xmlText, path);");
        out.println("      });");
        out.println("    }");
        out.println("    function parseXmlToTree(xmlText, path) {");
        out.println("      const parser = new DOMParser(); const xmlDoc = parser.parseFromString(xmlText, 'text/xml');");
        out.println("      const root = xmlDoc.documentElement;");
        out.println("      const filename = path.split('/').pop().replace('.pcf', '');");
        out.println("      const parsed = { id: root.getAttribute('id') || filename, title: root.getAttribute('title') || filename, children: [] };");
        out.println("      const parseChildren = (xmlNode) => {");
        out.println("        const list = [];");
        out.println("        for (let i = 0; i < xmlNode.children.length; i++) {");
        out.println("          const child = xmlNode.children[i];");
        out.println("          const nodeObj = { type: child.tagName, id: child.getAttribute('id') || (child.tagName + '_' + i), label: child.getAttribute('label'), value: child.getAttribute('value') };");
        out.println("          if (child.children.length > 0) { nodeObj.children = parseChildren(child); }");
        out.println("          list.push(nodeObj);");
        out.println("        }");
        out.println("        return list;");
        out.println("      };");
        out.println("      parsed.children = parseChildren(root); currentPcf = parsed; renderCanvas();");
        out.println("    }");
        out.println("    function cloneSelectedPcf() {");
        out.println("      const newId = prompt('Enter New PCF Identifier/Filename (e.g. CustomCommercialAutoSubmissionWizard):', currentPcf.id + 'Custom');");
        out.println("      if (!newId) return;");
        out.println("      currentPcf.id = newId; currentPcf.title = newId + ' Title';");
        out.println("      document.getElementById('canvasPcfName').innerText = 'Editing: New Template (' + newId + '.pcf)';");
        out.println("      renderCanvas();");
        out.println("      alert('Cloned successfully! Customize fields on the canvas and click Save to create ' + newId + '.pcf');");
        out.println("    }");
