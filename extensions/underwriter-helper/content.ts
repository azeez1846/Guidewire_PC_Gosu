...
const DEFAULT_TRIAGE_REQUEST: TriageRequest = {
...
function injectAICopilotOverlay(): void {
  if (document.getElementById("gw-ai-copilot-card")) return;
  ...
  const header: HTMLElement | null = card.querySelector("#gw-ai-header");
  const body: HTMLElement | null = card.querySelector("#gw-ai-body");
  const toggle: HTMLElement | null = card.querySelector("#gw-ai-toggle");
  let collapsed = false;

  header?.addEventListener("click", (): void => {
    collapsed = !collapsed;
    if (body) body.style.display = collapsed ? "none" : "block";
    if (toggle) toggle.innerText = collapsed ? "▲" : "▼";
  });

  const refreshBtn: HTMLButtonElement | null = card.querySelector("#gw-ai-refresh-btn");
...
function getBadgeStyles(recommendation: string): { background: string; color: string } {
...
function updateUI(data: TriageResponse): void {
  const badge = document.getElementById("gw-ai-badge");
  const scoreText = document.getElementById("gw-ai-score-text");
  const scoreBar = document.getElementById("gw-ai-score-bar") as HTMLElement | null;
  const rationaleList = document.getElementById("gw-ai-rationale");

  const score: number = data.riskScore ?? 20;
  const rec: string = data.recommendation ?? "STRAIGHT_THROUGH_BIND";
...
function applyFallbackUI(): void {
  const fallbackData: TriageResponse = {
...
function fetchUnderwritingTriage(): void {
  const badge = document.getElementById("gw-ai-badge");
...
chrome.runtime.onMessage.addListener(
  (
    ...
    sendResponse: (response: ExtractTextResponse | TriggerTriageResponse) => void
  ): void => {
